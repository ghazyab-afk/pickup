...
export default function DriverDashboardScreen({ navigation }: any) {
  ...
  const handleAcceptRide = async (ride: Ride) => {
    ...
    // Prévention de concurrence : On met à jour SEULEMENT si le statut est toujours 'pending'
    const { data, error } = await supabase
      .from('rides')
  ...
  const renderRide = ({ item }: { item: Ride }) => (
    <View className="bg-white rounded-2xl p-5 md:p-8 lg:p-10 mb-4 md:mb-6 shadow-sm border border-slate-100">
      <View className="flex-row justify-between items-start mb-4 md:mb-6">
        <View className="bg-blue-50 px-3 md:px-4 py-1 md:py-2 rounded-full border border-blue-100">
        ...
        <Text className="text-2xl md:text-3xl lg:text-4xl font-black text-green-600">
